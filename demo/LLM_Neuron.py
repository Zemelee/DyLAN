...
class LLMNeuron:
    ...
    def activate(self, question):
        ...
        self.active = True
        # contexts 系统提示词；formers (上一轮的回复,eid)
        ...
        # 构造formers + 任务描述prompt(评分)
        contexts.append(construct_message(formers, question, self.qtype))
        self.reply, self.prompt_tokens, self.completion_tokens = generate_answer(
            contexts, self.model
        )
        # parse answer
        ...
        for _, eid in formers:
            if 0 < weights[lp] <= 5:
                self.from_edges[eid].weight = weights[lp] / 5
            elif weights[lp] > 5:
                self.from_edges[eid].weight = 1
            else:
                self.from_edges[eid].weight = 0
            lp += 1
        # normalize weights
    ...
    def get_conversation(self):
        ...
        contexts, formers = self.get_context()
        contexts.append(
            construct_message([mess[0] for mess in formers], self.question, self.qtype)
        )
...
def listwise_ranker_2(responses, question, qtype, model="zhipu"):
    ...
    return (
        parse_ranks(completion, max_num=len(responses)),
        prompt_tokens,
        completion_tokens,
    )
